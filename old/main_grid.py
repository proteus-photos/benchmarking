...
if hasher.__name__ + ".npy" not in os.listdir("tile_databases") or args.refresh:
    ...
    original_hashes = [tile_hash for image in tqdm(images) for tile_hash in hasher(tilize(image, N_TILES))]
    ...
    database = TileDatabase(N_TILES, original_hashes, storedir=f"tile_databases/{hasher.__name__}", anchors=anchor_points_list)
else:
    database = TileDatabase(N_TILES, None, storedir=f"tile_databases/{hasher.__name__}")
