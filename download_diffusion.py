...
def unzip_file(file: str):
    ...
    for file in os.listdir("./diffusion_data"):
        if file.endswith(".png"):
            # convert to jpg using pillow
            image = Image.open(f"./diffusion_data/{file}")
            image.save(f"./diffusion_data/{file.replace('.png', '.jpg')}")
            os.remove(f"./diffusion_data/{file}")
...
def main(index=None, range_max=None, output=None, unzip=None, large=None):
    ...
    if index and range_max:
        if range_max - index >= 1999:
            ...
            if confirmation != "y":
                return
        download(index, range_max, output, large)
    elif index:
