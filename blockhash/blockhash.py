...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--bits", type=int, default=16, help="Create hash of size N^2 bits. Default: 16"
    )
    parser.add_argument(
        "--size", help="Resize image to specified size before hashing, e.g. 256x256"
    )
    parser.add_argument(
        "--interpolation",
        type=int,
    ...
    parser.add_argument(
        "--debug", action="store_true", help="Print hashes as 2D maps (for debugging)"
    )
    ...
    for fn in args.filenames:
        ...
        if args.debug:
            ...
            map = [
                bin_hash[i : i + args.bits] for i in range(0, len(bin_hash), args.bits)
            ]
            print("")
            print("\n".join(map))
            print("")
