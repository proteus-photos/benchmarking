...
def main(args):
    ...
    # save final model
    torch.save(model.model.state_dict(), f'{args.output_dir}/checkpoints/final.pt')
...
def train_one_epoch(
        ...
        normalize, args, epoch, dataloader_eval=None
):
    model_orig.eval()
    model.train()
    ...
    for i, (data, targets) in enumerate(dataloader):
        ...
        if args.save_checkpoints and (step_total % (args.steps // 10) == 0):
            ...
            torch.save(model.model.state_dict(), f'{args.output_dir}/checkpoints/step_{step_total}.pt')
        ...
        if step_total % 200 == 0:
            torch.save(model.model.state_dict(), f'{args.output_dir}/checkpoints/fallback_{step_total}.pt')
